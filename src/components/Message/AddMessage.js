...
function AddMessage({ channel_id = '' }) {
  ...
  const submitMessage = () => {
    ...
    /*
      Start hangman session
    */
    if (message.startsWith('/hangman')) {
      const re = /\/hangman+(\s\w*)?/;
      const found = message.match(re);
      if (!found || found.length > 9) {
        alert('Usage: /hangman <optional 8-letter word>');
      } else {
        var wordX = found
        axios.post(`/hangman/start`, { token, channel_id, wordX })
          .then(({ data }) => {
            alert(`A hangman session has begun!`);
          })
          .catch((err) => { });
      }
      return;
    ...
    */
    if (message.startsWith('/guess')) {
      ...
      const found = message.match(re);
      if (!found || found.length > 9) {
        alert('Usage: /guess <optional 1-8 letter subwords>');
      } else {
        var wordY = found
        axios.post(`/hangman/guess`, { token, channel_id, wordY })
          .then(({ data }) => {})
          .catch((err) => { });
      }
      return;
  ...
  // MAY NEED TO UPDATE THIS WITH HANGMAN?
  return (
    <>
      ...
      <div className={classes.flex}>
        <TextField
          ...
          label="Be Brave! Shout your message out loud! 💬"
          multiline
          placeholder="..."
